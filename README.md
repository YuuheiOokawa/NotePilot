...
2. **毎日バッチが未生成の記事を自動生成**（品質チェックまで。手動なら シリーズ詳細 → ⚡一括生成）
3. ホームの **「次にやること」** に従って ✅承認（1タップ）
4. **毎日バッチが承認済み記事へ投稿予定を自動割当**（翌日以降のJST 9〜21時に1時間1件で分散。手動なら 投稿キュー → ⚡自動割当）
5. `npm run poster` を起動しておくと、予定時刻に**無料記事は自動で公開**、有料記事は下書きまで自動投入（価格設定・公開のみ手動）。詳細と注意点は `docs/10_自動投稿ランナー.md`

> ⏰ 2と4はVercel Cronで毎日 9:00 JST に自動実行されます（`docs/11_定期実行バッチ.md`）。あなたの操作は「✅承認」だけです。
...
| 毎日バッチ | Vercel Cronが毎日、未生成記事の自動生成と投稿予定の自動割当を実行（承認は常にユーザー） |
...
| `CRON_SECRET` | - | 毎日バッチ(`/api/cron/daily`)の保護用。設定推奨（VercelがCronリクエストに自動付与） |
| `CRON_GENERATE_LIMIT` | - | 毎日バッチで自動生成する記事の最大件数（既定: 3） |
