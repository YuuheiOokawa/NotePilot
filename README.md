...
Next.js 14 (App Router) / TypeScript / Tailwind CSS / Prisma / PostgreSQL (Supabase) / Claude API（任意）
...
npm install
cp .env.example .env
# .env の DATABASE_URL に Supabase の接続文字列を設定
...
| 変数 | 必須 | 説明 |
|---|---|---|
| `DATABASE_URL` | ✅ | PostgreSQL接続文字列（Supabase: Project Settings > Database） |
